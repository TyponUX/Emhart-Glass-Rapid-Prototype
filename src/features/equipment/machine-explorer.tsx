...
export function MachineExplorer({ machineId, onBack, onRequestSupport }: MachineExplorerProps) {
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><Button variant="ghost" className="mb-2 -ml-3" onClick={onBack}><ArrowLeft className="mr-2 size-4" />All equipment</Button><h1 className="text-3xl font-semibold tracking-tight">{machine.name}</h1><p className="text-muted-foreground">{machine.model} · {machine.serialNumber} · {machine.site}</p></div><Badge variant="secondary">{machine.configuration}</Badge></div>
